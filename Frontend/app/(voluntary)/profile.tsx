...
export default function VoluntaryProfileScreen() {
  ...
  const [data, setData] = useState<VoluntaryMeResponse | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const { data: me } = await api.get<VoluntaryMeResponse>('/voluntary/me', { headers: { Authorization: `Bearer ${token}` } });
        setData(me);
  ...
  const dirty = useMemo(() => {
    ...
    return (
      ...
      JSON.stringify(skills) !== JSON.stringify(data.skills || [])
    );
  }, [phoneNumber, city, stateValue, skills, data]);
  ...
  async function handleSave() {
    ...
    try {
      setSaving(true);
      if(skills.length === 0){
        Alert.alert('Habilidades','Selecione pelo menos uma habilidade.');
        return;
      }
      const updatedResp = await api.patch<VoluntaryMeResponse>('/voluntary/update', {
    ...
    <SafeAreaView style={{ flex: 1 }} edges={['top','left','right']}>
      ...
      <View style={{ flex: 1 }}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 24, paddingTop: insets.top + 60, paddingBottom: (tabBarHeight + insets.bottom + 140) }}>
          ...
          {skills.length === 0 && (
            <Text style={{ marginTop:8, fontSize:12, fontFamily:'Nunito-Regular', color:'#C53030' }}>Selecione ao menos 1 habilidade.</Text>
          )}
        </ScrollView>
